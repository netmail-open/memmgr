...
static void _InitializeThreadLibrary( void )
{
	...
	if( !initState++ )
	{
		...
		MMStrcatf( &XplThreads.cacheText, __FILE__, __LINE__, "Thread Cache (%s)", __FILE__ );
	}
}
...
int XplThreadShutdown( void )
{
	...
	while( t = XplThreads.freeThreads.list )
	{
		...
		MMFree( t, __FILE__, __LINE__ );
	}
#endif

	_XplSemaDestroy( &XplThreads.work.semaphore );
	MMFree( XplThreads.cacheText, __FILE__, __LINE__ );
		...
		*tp = t->globalNext;
		...
		if( gr == XplThreads.staticGroup )
		{
			...
			if( !gr->threads )
			{
				...
				*gp = gr->next;
				XplLockRelease( &XplThreads.lock );
				MMFree( gr->userThreadGroup.identity, file, line );
				MMFree( gr, file, line );
			}
			else
			{
...
static void _Schedule( _XplThread *thread )
{
	...
	work = MMAllocEx( NULL, sizeof( _XplWork ), NULL, TRUE, FALSE, __FILE__, __LINE__ );
	MMWatch( work );
	work->thread = thread;
	work->next = NULL;
	XplLockAcquire( &XplThreads.work.lock );
...
static XplThreadGroup __XplThreadGroupCreate( const char *identity, XplBool locked, const char *file, unsigned long line )
{
	...
	errno = ENOMEM;
	if( group = MMAllocEx( NULL, sizeof( _XplThreadGroup ), NULL, FALSE, TRUE, file, line ) )
	{
		if( identity )
		{
			group->userThreadGroup.identity = MMStrdup( identity, NULL, TRUE, file, line );
		}
		else
		{
			group->userThreadGroup.identity = MMStrdup( "[static]", NULL, TRUE, file, line );
		}
...
EXPORT int _XplThreadStart( XplThreadGroup group, int (*userFunction)( XplThread ), void *context, XplThread *threadP, const char *file, unsigned long line )
{
	...
	{
		errno = ENOMEM;
		if( thread = MMAllocEx( NULL, sizeof( _XplThread ), NULL, FALSE, TRUE, file, line ) )
		{
	...
	{
		MMUpdateOwner( thread, file, line );
	}
	if( thread )
	{
...
EXPORT int _XplThreadFree( XplThread *thread, const char *file, unsigned long line )
{
	...
	if( thread && *thread )
	{
		...
		{
#ifdef CACHE_THREADS
			MMUpdateOwner( t, XplThreads.cacheText, __LINE__ );
...
EXPORT int XplThreadRegisterData( char *name )
{
	...
	userName = MemStrdupWait( name );
	XplLockAcquire( &XplThreads.lock );
	for(l=0;l<MAX_USER_DATA;l++)
	{
		if( XplThreads.userRegistration[l] )
		{
			if( !stricmp( userName, XplThreads.userRegistration[l] ) )
			{
				XplLockRelease( &XplThreads.lock );
				MemRelease( &userName );
				return l;
