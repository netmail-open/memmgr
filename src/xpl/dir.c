...
XplDir *XplOpenDir(const char *dirname)
{
	...
	if (!XplExpandEnv(expath, dirname, sizeof(expath))) {
		dirname = (const char *) expath;
	}

	if (dirname) {
		if ((newDir = (XplDir *)malloc(sizeof(XplDir)))) {
...
#elif defined(SOLARIS) || defined(LINUX) || defined(S390RH) || defined(MACOSX)
#include <grp.h>

int XplMakeDir(const char *path)
{
	struct group mplusGroup;
	struct group *result;
	char *buffer;
	long int bufsize;
	int r;

	if((r = mkdir(path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP)))
	{
		return (r);
	}

	bufsize = sysconf(_SC_GETGR_R_SIZE_MAX);
	/* this can return -1 if there is no hard limmit */
	if (bufsize < 0)
	{
		bufsize=2048;
	}
	...
	if (!getgrnam_r( NETMAIL_GROUP , &mplusGroup , buffer, bufsize, &result)
			&& result)
	{
		r = chown (path , -1, mplusGroup.gr_gid);
	}
#if 0
	else
	{
		XplConsolePrintf("Warning: group " NETMAIL_GROUP " does not exist.\n");
	}
#endif
	MemFree(buffer);
	return(0);
...
XplDirMatch * XplOpenDirMatch(const char *pattern)
{
	...
	char expath[XPL_MAX_PATH + 1];

	if (!XplExpandEnv(expath, pattern, sizeof(expath))) {
		pattern = (const char *) expath;
	}
...
EXPORT XplDir * XplOpenDir(const char *dirname)
{
	...
	if (dirname && !XplExpandEnv(expath, dirname, sizeof(expath))) {
		dirname = (const char *) expath;
	}
